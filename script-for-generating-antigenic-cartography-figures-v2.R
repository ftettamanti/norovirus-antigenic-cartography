...
ag.sr.plot.3D<-ag.sr.plot.3D %>% filter(!label %in% c("ctr.child1214","ctr.adult1214","ctr.adult1719"))%>%
  mutate(symbol=factor(case_when(
    label %in% c("child.1214","adult.1214","adult.1719","Den_Haag_2006")~"circle",
    label=="Sydney_2012" ~ "square",
    label=="Den_Haag_2017" ~ "triangle-up",
    label=="Osaka_2008" ~ "diamond",
    TRUE ~"triangle-down")))%>%
  mutate(label=factor(label,levels=c("child.1214","adult.1214","adult.1719","Sydney_2012","Den_Haag_2006","Den_Haag_2017","Osaka_2008","Hong_Kong_2019")))

coord.min.3D=floor(min(c(ag.sr.plot.3D$x,ag.sr.plot.3D$y,ag.sr.plot.3D$z)))-1
coord.max.3D=ceiling(max(c(ag.sr.plot.3D$x,ag.sr.plot.3D$y,ag.sr.plot.3D$z)))+1

axx <- list(
  nticks = 8,
  c(coord.min.3D,coord.max.3D)
)
...
fig <- plot_ly(ag.sr.plot.3D, x = ~x, y = ~y, z = ~z, 
               color=~label,
               colors = c("dodgerblue","darkolivegreen4","darkorchid3",viridis(5)),
               symbol=~type,symbols=c("square","circle"),type="scatter3d")
fig <- fig %>% layout(title = '3D Antigenic Cartography',
                      scene = list(xaxis = list(gridcolor = 'rgb(255, 255, 255)',
                                                range = c(coord.min.3D,coord.max.3D),
                                                zerolinewidth = 1,
                                                ticklen = 0,
                                                gridwidth = 2),
